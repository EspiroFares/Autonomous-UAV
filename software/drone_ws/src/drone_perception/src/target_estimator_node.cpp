...
public:
      ...
      has_reacquisition_candidate_(false)
    {
        ...
        // Camera/subject calibration for the pinhole range model. Defaults are
        // the real Pi-Camera + real-person values (unchanged behavior on
        // hardware). Override per camera — the Gazebo sim uses a different FOV
        // (focal 467.7 @ horizontal_fov 1.2) and a narrower-shouldered mannequin
...
private:
    void on_track(const drone_interfaces::msg::Track::SharedPtr msg) {
        ...
        const float known_shoulder_width =
